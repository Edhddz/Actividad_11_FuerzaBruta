...
int AdministradoraNeurona::dibujarConexionesCercanas(int& pos, int& x1, int& y1, int& x2, int& y2)
{
    ...
    while (i < contadorNeuronas){arregloDistancias[i] = 0; i++;}

    recuperaCoordenadaNeurona(pos, x1, y1);

    for(j = 0; j < contadorNeuronas; j++)//Obtener distancia entre puntos
    {
        if(j == pos){continue;}
        else
        {
    ...
    for(i = 0; i < contadorNeuronas; i++)//Encontrar indice menor
    {
        std::cout << "Valor inicial: " << arregloDistancias[indMenor] << std::endl;
        std::cout << "Valor en posicion: " << arregloDistancias[i] << std::endl;
        if(arregloDistancias[indMenor] == 0)
        {
            indMenor++;
            i++;
        ...
        else if(arregloDistancias[i] == 0 && i == contadorNeuronas-1)
        {
            std::cout << i <<std::endl;
            continue;
        }
        else if(arregloDistancias[i] == 0)
        {
            i++;
            if(arregloDistancias[i] < arregloDistancias[indMenor])
            {
        ...
        std::cout << "Valor iteracion: " << arregloDistancias[indMenor] << std::endl;
    }
    std::cout << "Valor menor: " << arregloDistancias[indMenor] << std::endl;
    for(int i = 0; i < contadorNeuronas; i++){std::cout << arregloDistancias[i] << std::endl;}
    return indMenor;

    //Implementar una forma de detectar que ya exita conexión entre ellas
}
...
void MainWindow::on_DibujarNPushButton_clicked()
{
    ...
    if(contadorNeurona > 0)
    {
        for(int i = 0; i < contadorNeurona; i++)
        {
            ...
            //std::cout << "PosX Neurona: " <<dNeurona.posX <<std::endl;
            //std::cout << "PosY Neurona: " <<dNeurona.posY <<std::endl;
        }
...
{
    if(admiNeurona.getContadorNeuronas() > 0){admiNeurona.sortNeuronas(true);}
    else{ui->InfoDisplay->setPlainText("No hay neuronas registradas..."); ui->tableWidget->setEnabled(false);}
}

void MainWindow::on_pushButton_2_clicked()
{
    if(admiNeurona.getContadorNeuronas() > 0){admiNeurona.sortNeuronas(false);}
    else{ui->InfoDisplay->setPlainText("No hay neuronas registradas..."); ui->tableWidget->setEnabled(false);}
}
...
void MainWindow::on_pushButton_3_clicked()
{
    ...
    if(admiNeurona.getContadorNeuronas() > 0)
    {
        for(int i = 0; i < contadorNeuronas; i++)
        {
